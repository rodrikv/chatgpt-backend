...
class MessageViewSet(viewsets.ModelViewSet):
    ...
    # authentication_classes = [JWTAuthentication]
    permission_classes = [IsAuthenticated, IsOwnerOfChat]
...
def conversation(request):
    ...
    def stream_content():
        ...
        yield sse_pack(
            "done",
            {"messageId": ai_message_obj.id, "conversationId": conversation_obj.id},
        )

    response = StreamingHttpResponse(stream_content(), content_type="text/event-stream")
...
def create_message(
    user, conversation_id, message, is_bot=False, messages="", tokens=0, api_key=None
):
    message_obj = Message(
...
def build_messages(
    ...
    result = {"messages": messages, "tokens": 0}
